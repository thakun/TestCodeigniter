...
function cargar_dropdowns(){
	$.post("asignaciones/cargar_dropdowns",{},function(respuesta) {
...
function cargar_asignaciones() {
	$.post("asignaciones/cargar",{},function(respuesta) {
		respuesta = JSON.parse(respuesta);
		...
		$.each(respuesta,function(i,j) {
			...
			tr.append('<td><button id="'+j.id_asignacion+'" class="btnImg btn btn-danger"><img src="assets/img/eliminar.png"></button>' )
			tr.appendTo($('tbody'));

			$("#"+j.id_asignacion).click(function(e) {
				$.post("asignaciones/eliminar",{matricula:j.matricula,clave:j.clave_materia},function(respuesta) {
